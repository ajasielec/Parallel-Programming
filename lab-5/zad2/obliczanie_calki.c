...
double calka_sekw(double a, double b, double dx){
  int N = ceil((b - a) / dx);     // liczba trapezów potrzebnych, by objąć przedział [a, b]
  double dx_adjust = (b-a)/N;     // dostosowanie dx (szerokosc trapezu), aby pasowało do przedziału
  ...
  // petla sumujaca pola trapezow
  for(i = 0; i < N; i++){
    double x1 = a + i * dx_adjust;    // lewy kraniec trapezu
    double x2 = x1 + dx_adjust;       // prawy kraniec trapezu
    double y1 = funkcja(x1);          // wartosc funkcji w x1
    double y2 = funkcja(x2);          // wartosc funkcji w x2

    // obliczanie pola trapezu i dodanie do sumy
    calka += 0.5 * dx_adjust * (y1 + y2);
